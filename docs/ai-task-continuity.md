...
- **Status**: Completed T070-T072: student/enrollment and parent link/delegation schemas, attendance/score migration, enrollment service, progress/attendance/score completion states, and parent delegation grant/revoke/expiry scoped resolver.
- **Next task**: T073, implement student/parent portals for schedule, progress, scores, tuition and homework in `apps/web/src/app/admin/student/` and `apps/web/src/app/admin/parent/`.
...
- **Files in focus**: `apps/web/src/app/admin/student/`, `apps/web/src/app/admin/parent/`, shared UI table/layout rules.
- **Verification**: `npm run typecheck --workspace=@hnlms/api` passed; `npm test -- --run` passed (328 tests).
