...
import { AssignmentMutationError, AssignmentService, type AssignmentRepository } from "../assignment.service";
...
describe("AssignmentService", () => {
  it("rejects mutations outside the actor organization", async () => {
    ...
    await expect(
      service.createScopeGrant({ userId: "admin", organizationId: "org-a" }, {
        userId: "member",
        organizationId: "org-b",
        kind: "branch",
        resourceId: "branch-1",
      }),
    ).rejects.toMatchObject<AssignmentMutationError>({ code: "forbidden" });
  ...
  it("writes role replacements and audits the before/after values", async () => {
    ...
    await service.replaceUserRoles({ userId: "admin", organizationId: "org-a" }, {
      userId: "member",
  ...
  it("rejects an invalid grant period before persistence", async () => {
    ...
    await expect(service.createScopeGrant({ userId: "admin", organizationId: "org-a" }, {
      userId: "member",
      organizationId: "org-a",
      kind: "student",
