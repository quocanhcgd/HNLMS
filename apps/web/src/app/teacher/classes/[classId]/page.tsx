...
export default function TeacherClassDetailPage() {
  ...
  return <div className="page">
    ...
    <div style={{ display: "grid", gridTemplateColumns: "minmax(190px, 240px) minmax(0, 1fr)", gap: 20, alignItems: "start" }}>
      <Paper className="panel" p="xs" withBorder><Stack gap={4}>{tabs.map((tab) => { const Icon = tab.icon; return <Link key={tab.key} href={`/teacher/classes/if-2609/${tab.key}`} className={`navItem ${tab.key === activeKey ? "active" : ""}`}><Icon size={16}/><span>{tab.label}</span></Link>; })}</Stack></Paper>
      <Paper className="panel" p="lg" withBorder><Group justify="space-between" mb="md"><div><Text fw={700}>{active.label}</Text><Text size="sm" c="dimmed">{active.summary}</Text></div><UiButton variant="default">Làm mới dữ liệu</UiButton></Group><Paper className="panelHighlight" p="lg"><Text fw={700}>Luồng xử lý hiện tại</Text><Text size="sm" c="dimmed" mt="xs">Trước buổi dạy: chuẩn bị học liệu. Trong buổi dạy: điểm danh và chấm bài trên lớp. Sau buổi dạy: chấm bài về nhà, xác nhận worklog và gửi thông tin cho học vụ/payroll.</Text></Paper><Group mt="lg"><UiButton>Thực hiện hành động chính</UiButton><UiButton variant="default" component={Link} href="/teacher/worklog">Mở worklog buổi dạy</UiButton></Group></Paper>
    </div>
  </div>;
}
